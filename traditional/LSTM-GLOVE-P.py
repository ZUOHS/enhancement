...
def read_data(fold_id, FLAG, csv_file='../data/enhancement.csv'):
    ...
    return (text_train, text_val, text_test,
            ...
            creator_train, creator_val, creator_test,
            role_train, role_val, role_test,
            freq_train, freq_val, freq_test, id_test)
...
def LSTM_Model(text_train, text_val, text_test,
               y_train, y_val, y_test,
               creator_train, creator_val, creator_test,
               role_train, role_val, role_test,
               freq_train, freq_val, freq_test, id_test):
    ...
    role_all = to_categorical(role_train + role_val + role_test)
    role_train, role_val, role_test = role_all[:len(role_train)], role_all[len(role_train):len(role_train) + len(role_val)], role_all[len(role_train) + len(role_val):]
    
    encoder = LabelEncoder()
    encoder.fit(creator_train + creator_val + creator_test)
    creator_all = to_categorical(encoder.transform(creator_train + creator_val + creator_test))
    creator_train, creator_val, creator_test = creator_all[:len(creator_train)], creator_all[len(creator_train):len(creator_train) + len(creator_val)], creator_all[len(creator_train) + len(creator_val):]
    ...
    creator_train, creator_val, creator_test = map(np.array, [creator_train, creator_val, creator_test])
    role_train, role_val, role_test = map(np.array, [role_train, role_val, role_test])
    freq_train, freq_val, freq_test = map(np.array, [freq_train, freq_val, freq_test])
    
    for dataset in [(text_train, y_train, creator_train, role_train, freq_train),
                    (text_val, y_val, creator_val, role_val, freq_val),
                    (text_test, y_test, creator_test, role_test, freq_test)]:
    ...
    role_inputs = Input(shape=(role_train.shape[1],), name='role_inputs')
    creator_inputs = Input(shape=(creator_train.shape[1],), name='creator_inputs')
    ...
    concatenation = concatenate([lstm_layer, creator_inputs, role_inputs, freq_inputs])
    concatenation_reduced = Dense(12800, activation='relu')(concatenation)
    attention_probs = Dense(12800, activation='softmax', name='attention_vec')(concatenation_reduced)
    attention_mul = Multiply()([concatenation_reduced, attention_probs])
    
    dropout = Dropout(0.5)(attention_mul)
    ...
    model = Model(inputs=[inputs, role_inputs, creator_inputs, freq_inputs], outputs=predictions)
    ...
    history = model.fit([text_train_pad, role_train, creator_train, freq_train], y_train,
                        batch_size=batch_size, epochs=epoch, verbose=1,
                        callbacks=[early_stopping, reduce_lr],
                        validation_data=([text_val_pad, role_val, creator_val, freq_val], y_val))
    
    score = model.evaluate([text_test_pad, role_test, creator_test, freq_test], y_test, verbose=1)
    y_predict = model.predict([text_test_pad, role_test, creator_test, freq_test])
    auc = metrics.roc_auc_score(y_test, y_predict)
    y_predict_1 = (y_predict >= 0.5).astype(int)
    precision, recall, f, _ = metrics.precision_recall_fscore_support(y_test, y_predict_1, beta=1.0, pos_label=1, average=None)
    cm = metrics.confusion_matrix(y_test, y_predict_1)
    ...
    test_results_df = pd.DataFrame({
        "id": id_test, 
        "true_label": y_test,
        "predicted_label": y_predict.flatten(),
        "predicted_prob": y_predict_1.flatten()
    })
...
for i in range(1):
    (text_train, text_val, text_test,
            ...
            creator_train, creator_val, creator_test,
            role_train, role_val, role_test,
            freq_train, freq_val, freq_test, id_test) = read_data(1, FLAG, csv_file='../data/enhancement.csv')

    precision, recall, f, score, auc = LSTM_Model(text_train, text_val, text_test,
            y_train, y_val, y_test,
            creator_train, creator_val, creator_test,
            role_train, role_val, role_test,
            freq_train, freq_val, freq_test, id_test)
...
print("Class 1 - Precision: {}, Recall: {}, F1: {}".format(float(total_p1), float(total_r1),
                                                           float(total_f1)))
print("Class 0 - Precision: {}, Recall: {}, F1: {}".format(float(total_p0), float(total_r0),
                                                           float(total_f0)))
